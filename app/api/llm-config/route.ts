  ...
  try {
    ...
    const apiKey =
      typeof body?.apiKey === "string"
        ? mergeApiKeyFromClient(body.apiKey, current.apiKey)
        : current.apiKey;
    ...
    saveLlmConfig({ apiKey, models });
    return NextResponse.json({ ok: true, ...getLlmConfigPublic() });
  } catch (e) {
    return NextResponse.json({ error: String(e) }, { status: 500 });
  }
}
